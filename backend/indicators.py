"""
StockPulse Quantitative Technical Indicators Engine (Pure Python)
Provides high-performance computation for RSI, MACD, Bollinger Bands, and Moving Averages.
"""
...
def calculate_ema(prices: List[float], period: int) -> List[Optional[float]]:
    ...
    # Initial SMA for the seed value
...
def calculate_rsi(prices: List[float], period: int = 14) -> Dict[str, Any]:
    ...
    Standard period: 14. Scale: 0-100.
    Overbought >= 70, Oversold <= 30.
    """
    if len(prices) < period + 1:
        # Graceful fallback for short histories
        return {
            "rsi": 50.0,
            "condition": "NEUTRAL",
            "period": period,
...
def calculate_macd(
    ...
    # Calculate MACD line where both fast and slow EMAs exist
    ...
    # Signal line is EMA of the valid MACD line values
    ...
    trend = "BULLISH_EXPANSION" if histogram > 0 and current_macd > 0 else "BEARISH_CONTRACTION" if histogram < 0 else "CONSOLIDATING"
...
def calculate_bollinger_bands(
    ...
    """
    Calculates Bollinger Bands (Middle Band, Upper Band, Lower Band, Bandwidth, %B).
    """
    if len(prices) < period:
