"""
StockPulse Python AI Financial Copilot
Integrates Google Gemini 2.5 Flash via official google-genai SDK with Search Grounding,
resilient dual fallback, prompt injection fencing, and quantitative heuristics.
"""
    ...
    if _client is None:
        ...
        if api_key and api_key != "MY_GEMINI_API_KEY":
            try:
                ...
                print(f"[StockPulse Copilot] Failed to initialize google-genai client: {e}")
                _client = None
    return _client
...
def run_copilot_inference(
    ...
    """
    Executes financial intelligence inquiry through Gemini 2.5 Flash with Search Grounding,
    or falls back gracefully to quantitative heuristics.
    """
...
"""
            # Try Gemini 2.5 Flash with search grounding
            try:
                ...
                if response and response.text:
                    ...
                    model_used = "gemini-2.5-flash (Google Search Grounded)"
                ...
                # Fallback to gemini-2.0-flash without search grounding
                try:
                    response = client.models.generate_content(
                    ...
                    print(f"[StockPulse Copilot] Cloud inference fallback error: {e2}")

        except Exception as e:
            print(f"[StockPulse Copilot] GenAI invocation error: {e}")

    # Fallback to quantitative heuristics if AI inference is unavailable
    if not answer:
...
1. **Layer 1: Python Pytest Suite (12+ Unit & Integration Tests)**
